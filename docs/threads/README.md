...
- [0009 — Secrets vault backend + `get_secret`](0009-secrets-vault-getsecret.md) — 🛠️ **BUILT, pre-apply review** (Supabase Vault). Design approved-with-corrections; all 8 addressed in migration `0010` (split authed/operator reads, sensitivity authz, admin-only set_secret, no metadata-write bypass, project-aware unique identity + stable vault name, NOT-NULL/UNIQUE vault pointer, direct-vault revoke incl. service_role, atomic log_activity audit, fail-closed preflight, corrected backups claim). `0010` UNAPPLIED; no real secret ingested. MCP `get_secret` tool = separate later slice.

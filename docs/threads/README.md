...
- [0009 — Secrets vault backend + `get_secret`](0009-secrets-vault-getsecret.md) — 🛠️ **BUILT (r3), pre-apply re-review** (Supabase Vault). Design+8 + 3 pre-apply blockers done; r2's lifecycle blocker fixed (retire ordering: audit→metadata→vault, raise on zero rows, atomic). `0010` UNAPPLIED; no real secret ingested. MCP `get_secret` tool = separate later slice.

...
- **Active Builds roster** — one line per project: `name → path | branch | topic file`. Drives `/switch`.
...
**`/switch <project>`** (`~/.claude/commands/switch.md`, or the `switch` skill) — manual command you type after `/clear`. (Renamed from `/resume`, which collided with Claude Code's built-in session-resume — don't use `/resume`.) It resolves the name in the roster → reads the topic file's `⭐ RESUME` block → runs `git status` + `git log` → reports where-we-are / state / next-action / guardrails in ≤12 lines, then waits. Loads one project's context per switch, not all of them every turn. (It's manual because a shared working root can't infer which project you mean.)
...
**OPTIONAL mirror (multi-dev/shared repos only):** an in-repo `docs/CHECKPOINT.md` mirroring the `⭐ RESUME` block, so co-devs + their agents see it. It **never replaces** the memory file. `/switch` reads the memory file first, then the in-repo checkpoint if present.

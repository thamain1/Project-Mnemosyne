-- Mnemosyne — 0029: second gate-run fix for recall_memory_hybrid (thread 0032, 2026-07-03; applied
-- same day). The similarity expression (rrf * recency boost) evaluates as NUMERIC (1.0/(60+rank) is
-- numeric math), mismatching the declared `similarity double precision` OUT column -> 42804 on every
-- call. Masked by 0028's 42702 until that was fixed. Cast the final expression. This file is the
-- authoritative current definition of recall_memory_hybrid.
...
begin
  ...
  select
    ...
    (fused.rrf * (1 + 0.1 * exp(-(extract(epoch from (now() - fused.updated_at)) / 86400.0) / 90.0)))::double precision as similarity,
